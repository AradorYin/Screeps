...
export function loop() {
    ...
    if(harvesters.length < 2) {
        ...
        Game.spawns['Spawn1'].spawnCreep([WORK,CARRY,MOVE], newName, {memory: {role: memoryCreep.UPGRADER}});
    }
    // else if(builders.length < 1) {
    //     const newName = 'Builder' + Game.time;
    //     console.log('Spawning new builder: ' + newName);
    //     Game.spawns['Spawn1'].spawnCreep([WORK,CARRY,MOVE], newName, {memory: {role: memoryCreep.BUILDER}});
    // }
    ...
    for(const name in Game.creeps) {
        ...
        if(creep.memory.role === memoryCreep.HARVESTER) {
            roleHarvester.run(creep);
        }
        if(creep.memory.role === memoryCreep.UPGRADER) {
            roleUpgrader.run(creep);
        }
        if(creep.memory.role === memoryCreep.BUILDER) {
